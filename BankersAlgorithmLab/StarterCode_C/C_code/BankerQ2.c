...
int checkSafe(int customerIndex, int *request) {
	...
	/*
		The rationale for assuming request is granted here instead of in requestResources() 
		is because, it would be easier to mess around with temporaries copies of the data 
		structures and check for safety, rather than change the original data structure, 
		check for safety, and change them back if not safe.
	*/
	// Assume request is granted, update temporary arrays
	for(int i=0; i<numberOfResources; i++){
		work[i] -= request[i];
	...
	// Initialise all customers to be not finished
	for(int i=0; i<numberOfCustomers; i++){
		finish[i]=0;
	}
	...
	/*
		The purpose of the while loop is to continually loop through the data structures. The 
		purpose of the first 2 for loops are to check if a customer can be finished in one while
		loop iteration. If a customer can be finished, advance to the next while loop iteration.
		If not, while loop is broken.

		The system is safe if all customers can be finished.
	*/

	// return value, to say if bank is safe after request granted
	int safe=1;
	// Allows the while loop to continue looping, if a customer can finish in an iteration of the internal for loop
	int advance=1;		

	while(advance){
		advance=0;
		for(int i=0; i<numberOfCustomers; i++){
			int proceed=1;									// Indicates this customer can finish, and can proceed to update resources
			for(int j=0; j<numberOfResources; j++){
				if(tempNeed[i][j]>work[j]){
					// Cannot proceed to update requests since customer cannot finish
					proceed=0;
				}
			}

			// If customer is not finished, and can finish
			if(finish[i]==0 && proceed==1){
				finish[i]=1;
				advance=1;									// Since a customer is finished, allow for another iteration of the for loop
				
				// Since customer finished, deallocate the resources used
				for(int j=0; j<numberOfResources; j++){	
	...
	// Checks if all customers are finished
	for(int i=0; i<numberOfCustomers; i++){
		if(finish[i]==0){
			safe = 0;
		}
...
int requestResources(int customerIndex, int *request) {
	...
	// TODO: judge if the new state is safe if grants this request (for question 2)
		// I thought that this TODO would be better done in the checkSafe function itself
...
void releaseResources(int customerIndex, int *release) {
	...
	// Updates the necessary arrays once customer returns the resources to the bank
	for(int i=0; i<numberOfResources; i++){
		available[i]+=release[i];								// Increase the available resources for other customers
		allocation[customerIndex][i]-=release[i];				// Decrease the allocated resource of the specific customer
		need[customerIndex][i]+=release[i];						// Decrease the overall need of the specific customer
	}
}
