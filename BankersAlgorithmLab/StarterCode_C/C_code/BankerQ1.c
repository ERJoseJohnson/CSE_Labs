...
int checkSafe(int customerIndex, int *request) {
	...
	// return value
	int safe=1;
	// Loops the while loop as long as a process can finish in one full customer for_loop
	int advance=1;		

	while(advance){
		advance=0;
		for(int i=0; i<numberOfCustomers; i++){
			int proceed=1;		// Indicates that this process can finish, can proceed to increase work
			for(int j=0; j<numberOfResources; j++){
			...
			// If process is not already finished, and can finish
